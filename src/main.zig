...
fn makeKernelStandardEnvironment(bank: *Sexpr.Bank) Sexpr {
    ...
    var parser: Parser = .{ .remaining_text = 
        ...
        \\
        \\ // returns (nibble . rest)
        \\ ($define! take-nibble ($lambda (binary-digits) (asdfasdfasdf binary-digits ($quote (b0 b0 b0 b0)))))
        \\ ($define! asdfasdfasdf ($lambda (source fill) ($if (empty? source) (cons fill ()) ($if (empty? fill) (cons () source) 
        \\   ($let ((s1 . sR) source) 
        \\     ($let ((_ . fR) fill) 
        \\       ($let ((xx . yy) (asdfasdfasdf sR fR))  (cons (cons s1 xx) yy))))))))
        \\
        \\ ($define! hex-from-nibble ($lambda (nibble) ($cond
        \\   ((=? nibble ($quote (b0 b0 b0 b0))) ($quote x0))
        \\   ((=? nibble ($quote (b1 b0 b0 b0))) ($quote x1))
        \\   ((=? nibble ($quote (b0 b1 b0 b0))) ($quote x2))
        \\   ((=? nibble ($quote (b1 b1 b0 b0))) ($quote x3))
        \\   ((=? nibble ($quote (b0 b0 b1 b0))) ($quote x4))
        \\   ((=? nibble ($quote (b1 b0 b1 b0))) ($quote x5))
        \\   ((=? nibble ($quote (b0 b1 b1 b0))) ($quote x6))
        \\   ((=? nibble ($quote (b1 b1 b1 b0))) ($quote x7))
        \\   ((=? nibble ($quote (b0 b0 b0 b1))) ($quote x8))
        \\   ((=? nibble ($quote (b1 b0 b0 b1))) ($quote x9))
        \\   ((=? nibble ($quote (b0 b1 b0 b1))) ($quote xA))
        \\   ((=? nibble ($quote (b1 b1 b0 b1))) ($quote xB))
        \\   ((=? nibble ($quote (b0 b0 b1 b1))) ($quote xC))
        \\   ((=? nibble ($quote (b1 b0 b1 b1))) ($quote xD))
        \\   ((=? nibble ($quote (b0 b1 b1 b1))) ($quote xE))
        \\   ((=? nibble ($quote (b1 b1 b1 b1))) ($quote xF))
        \\ )))
        \\ ($define! hex-from-binary ($lambda (binary-digits)
        ...
        \\ ($define! + ($lambda (a b) (hex-from-binary (binary-add (binary-from-text a) (binary-from-text b)))))
        \\ 
    };
    while (parser.next(bank) catch @panic("bad text")) |v| {
